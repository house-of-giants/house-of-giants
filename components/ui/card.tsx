...
function Card({ className, size = 'default', ...props }: React.ComponentProps<'div'> & { size?: 'default' | 'sm' }) {
	return (
		<div
			...
			data-size={size}
			className={cn(
				'border-border bg-card text-card-foreground group/card hover:border-primary/50 flex flex-col gap-4 overflow-hidden rounded-none border py-4 text-xs/relaxed transition-colors has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-none *:[img:last-child]:rounded-none',
				className
			)}
			{...props}
		/>
