...
import { Code, Palette, Users, Compass, Rocket, Target, Package, ArrowUpRight, TrendingUp } from 'lucide-react';
		...
		<main>
			...
			<Section className="relative">
				<SectionHeader
					...
					description={
						<>
							Everything you need to launch a custom web application that actually works. Strategy, design, development,
							and ongoing support. Delivered via{' '}
							<Link href="/dedicated-teams" className="text-primary hover:underline">
					...
					{services.map((service) => {
						...
						return (
							<div
								key={service.id}
								id={service.id}
								className="bg-background group border-border hover:border-primary/50 relative flex scroll-mt-32 flex-col overflow-hidden border p-8 transition-all hover:shadow-lg lg:p-10"
							>
								...
								<div className="mb-8">
									<h4 className="text-foreground mb-4 text-sm font-semibold tracking-wider uppercase">Deliverables</h4>
								...
								<div className="bg-muted/30 -mx-8 mt-auto -mb-10 border-t p-8 lg:-mx-10 lg:-mb-10 lg:p-10">
									...
									<div className="mb-6 flex flex-wrap gap-2">
										{service.goodFor.map((item) => (
											<span
												key={item}
												className="bg-background border-border text-muted-foreground inline-flex border px-3 py-1 text-xs font-medium"
											>
												{item}
											</span>
										))}
									...
									<Link
										href="/work"
										className="text-primary group/link mt-6 inline-flex items-center gap-2 text-sm font-medium"
									>
				...
				<div className="grid gap-6 sm:grid-cols-2">
					{processPhases.map((phase, index) => {
						...
						return (
							<div
								key={phase.label}
								className="bg-background/50 border-border hover:border-primary/50 group relative flex flex-col border p-8 backdrop-blur-sm transition-all hover:shadow-lg"
							>
								<div className="mb-6 flex items-center justify-between">
									<span className="bg-primary/10 text-primary font-display flex size-10 items-center justify-center text-base font-bold">
										{index + 1}
									</span>
								...
								<div className="border-border mb-4 border-t pt-4">
									<p className="text-muted-foreground mb-2 text-xs font-medium tracking-wider uppercase">Our job</p>
				...
				<div className="border-border mt-12 border-t pt-8">
					<div className="flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
						<div className="text-center sm:text-left">
							...
							<p className="text-muted-foreground text-sm">Real projects. Real timelines. Real outcomes.</p>
						</div>
						<Button href="/work" variant="default" size="cta">
							View case studies
			...
			<Section>
				<SectionHeader
					eyebrow="Philosophy"
					title="We choose tools."
					subtitle="Tools don't choose us."
				...
				<div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-6">
					{approachItems.map((item) => (
						<div
							key={item.title}
							className="bg-background border-border hover:border-primary/50 group border p-6 transition-colors"
						>
			...
			<CaseStudyRelated
				...
				description="We don't just talk about our process...we ship it. Here's proof."
				showCta
