...
import { Globe, Layers, Code, Check, Sparkles, Clock, Puzzle, FileText, Headphones, MessageSquare } from 'lucide-react';
...
export default function PricingContent() {
	const [selectedType, setSelectedType] = React.useState<ProjectType['id'] | null>(projectTypes[1].id);
			...
			<Section>
					...
					{projectTypes.map((type, index) => {
						...
						return (
							<button
								...
								className={cn(
									'bg-background border-border group relative flex cursor-pointer flex-col items-center border p-8 text-center transition-all duration-300',
							...
							>
								...
								<div
									...
									style={{
										background: 'radial-gradient(600px circle at 50% 0%, oklch(0.85 0.22 170 / 15%), transparent 60%)',
									}}
								/>

								<div
									className={cn(
										'border-border bg-card mb-6 inline-flex size-14 items-center justify-center border transition-colors',
				...
				>
					<div className="min-h-0 overflow-hidden">
						...
						>
							{recommendedTier && (
								<div className="border-primary/30 bg-primary/5 mx-auto max-w-2xl border p-8">
				...
				<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{pricingFactors.map((factor, index) => {
						...
						return (
							<div
								key={factor}
								className="bg-background border-border hover:border-primary/50 flex items-center gap-4 border p-6 transition-colors"
							>
